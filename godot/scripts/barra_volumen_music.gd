...
# Señal que se emite cuando cambia el volumen de la música, pasando el nuevo valor (0 a 1)
signal volumen_cambiado(nuevo_valor)

# Nodo que detecta clics y movimientos del ratón
@onready var detector_clics = $DetectorClics
# Contenedor que agrupa los cuadros visuales que representan el nivel de volumen
@onready var contenedor_cuadros = $ContenedorCuadros

var cuadraditos : Array = [] # Lista para guardar los cuadros que indican el volumen
...
# Actualiza la visibilidad de los cuadros según el valor de volumen (de 0 a 1)
func actualizar_visuales(valor_0_a_1 : float):
	var total_cuadros = cuadraditos.size()
	# Calculamos cuántos cuadros deben estar visibles según el volumen actual
	var cuadros_a_encender = round(valor_0_a_1 * total_cuadros)
	
	for i in range(total_cuadros):
		if i < cuadros_a_encender:
			cuadraditos[i].visible = true # Activamos el cuadro
		else:
			cuadraditos[i].visible = false # Desactivamos el cuadro

# Detecta clics y arrastres para cambiar el volumen
func _on_detector_gui_input(event):
	# Comprobamos si se hizo clic izquierdo o se está arrastrando con el botón izquierdo presionado
	...
	if click_izquierdo or arrastrando:
		var ancho_total = detector_clics.size.x # Ancho total del área clicable
...
func set_valor(valor_0_a_1):
	valor_actual = clamp(valor_0_a_1, 0.0, 1.0) # Limitamos el valor al rango válido
	actualizar_visuales(valor_actual) # Actualizamos los cuadros visuales
